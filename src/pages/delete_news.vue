  ...
  <section class="news-page">
    <div class="news-container">
      <header class="news-header">
        <h1 class="news-title reveal" :class="{ 'is-in': pageEnter }" :style="{ '--d': '80ms' }">News</h1>
        <p class="news-subtitle reveal" :class="{ 'is-in': pageEnter }" :style="{ '--d': '140ms' }">
          {{ i18n.locale === 'zh' ? '选择要删除的新闻（仅此页提供删除）。' : 'Select a news item to delete (delete only available on this page).' }}
        </p>
      </header>
      ...
      <div class="grid" role="list" :aria-label="t('news.ariaList')">
        ...
        <div v-else-if="filtered.length === 0" class="empty" role="status" aria-live="polite">
          No articles yet.
        </div>
        ...
        <article
          ...
          :key="item.id"
          class="card reveal"
          :class="[{ 'is-selected': String(item.id) === selectedId }, { 'is-in': pageEnter }]"
        ...
        >
          <button
            ...
            type="button"
            @click="select(item)"
            :aria-label="`Select to delete: ${item.title}`"
          >
      ...
      <div class="delete-bar" role="region" aria-label="Delete actions">
        <div class="delete-inner">
          <div class="delete-left">
            <div class="delete-title">
              {{ i18n.locale === 'zh' ? '已选择：' : 'Selected: ' }}
              <span class="delete-title__name">{{ selectedTitle || (i18n.locale === 'zh' ? '（未选择）' : '(none)') }}</span>
            </div>
            <label class="delete-confirm">
              <input class="delete-checkbox" type="checkbox" v-model="confirm" :disabled="!selectedId || isDeleting || !canManage" />
              <span class="delete-confirm-text">
                {{ i18n.locale === 'zh' ? '我确认删除不可撤销' : 'I understand this cannot be undone' }}
              </span>
            </label>
            <div v-if="!canManage" class="delete-hint">
              {{ i18n.locale === 'zh' ? '没有权限或未登录：只能浏览，不能删除。' : 'No permission / not logged in: browse only, cannot delete.' }}
            </div>
          </div>

          <div class="delete-right">
            <button class="btn ghost" type="button" @click="router.push({ name: 'news' })">
              {{ i18n.locale === 'zh' ? '返回新闻列表' : 'Back to News' }}
            </button>
            <button class="btn ghost" type="button" @click="clearSelection" :disabled="isDeleting">
              {{ i18n.locale === 'zh' ? '清除选择' : 'Clear' }}
            </button>
            <button
              class="btn danger"
              type="button"
              @click="doDelete"
              :disabled="isDeleting || !selectedId || !confirm || !canManage"
            >
              {{ isDeleting ? (i18n.locale === 'zh' ? '删除中…' : 'Deleting…') : (i18n.locale === 'zh' ? '删除' : 'Delete') }}
            </button>
          </div>
        </div>
      </div>
...
import { deleteNews, listNews } from '@/services/newsService'
...
function select(item) {
  ...
  selectedId.value = id
  selectedTitle.value = String(item?.title || '')
  confirm.value = false
}
...
async function doDelete() {
  ...
  try {
    await deleteNews(id)
    successStore.showSuccess(t('user.newsDeleteSuccess') || (i18n.locale === 'zh' ? '删除成功' : 'Deleted'))
    clearSelection()
    await loadNews()
  } catch (e) {
    errorStore.showError(e?.message || (i18n.locale === 'zh' ? '删除失败' : 'Failed to delete'))
  } finally {
...
/* Copied from News.vue to keep page identical */
.news-page {
  background: #fbfbfb;
...
.grid {
  ...
  padding-bottom: 96px; /* leave room for delete bar */
}
...
/* Only difference vs News.vue: selection highlight */
